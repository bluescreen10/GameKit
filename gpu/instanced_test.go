...
func extractPlanes(vp mat4) [6][4]float32 {
...
func renderInstances(b gpu.Backend, size int, offsets [][3]float32, eye vec3) (px []byte, survivors uint32) {
	...
	ms := unsafe.Slice((*mat4)(models.Ptr), n)
	...
	*args = drawIndexedIndirect{indexCount: uint32(indexCount)}
	...
	proj := perspectiveRH(float32(50*math.Pi/180), 1, 0.1, 100)
	view := lookAtRH(eye, vec3{0, 0, 0}, vec3{0, 1, 0})
	vp := mul4x4(proj, view)
...
func TestInstancedGPUDriven(t *testing.T) {
	b := testBackend(t)
	const size = 160
	px, survivors := renderInstances(b, size, [][3]float32{{-0.8, -0.8, 0}, {0.8, -0.8, 0}, {-0.8, 0.8, 0}, {0.8, 0.8, 0}}, vec3{0, 0, 4})
	if survivors != 4 {
...
func TestFrustumCull(t *testing.T) {
	...
	px, survivors := renderInstances(b, size, offsets, vec3{0, 0, 4})
